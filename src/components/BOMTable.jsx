...
// ── 열 관련 상수 ──────────────────────────────────────────────
const EBOM_FIXED_KEYS = new Set([
  '_drag', 'seq', 'level', 'no', 'childPart', 'description', '_purchase', 'unitQty', 'qtyTotal', '_link',
]);

const EBOM_ALL_OPTIONAL_KEYS = [
  'itemType', 'staNo', 'processType', 'drawingDate', 'parentPart', 'rev',
  'vendor', 'material', 'spec', 'sizeT', 'sizeW', 'sizeL', 'weight',
  'unit', 'multiplier', 'qtyPerOne', 'remark',
];

// 부품 행에서 drawing DB에 영속화할 편집 가능 필드
const DRAWING_PERSIST_EDITABLE = new Set(['staNo', 'processType', 'spec']);

// ── ColumnManager ─────────────────────────────────────────────
function ColumnManager({ colDefs, visible, onToggle }) {
  ...
  const optional = colDefs.filter((c) => !EBOM_FIXED_KEYS.has(c.key) && c.label);
...
export default function BOMTable({ isDark = false, searchRef }) {
  ...
  const [colVisible, setColVisible] = useState(() => {
    ...
    return new Set(EBOM_ALL_OPTIONAL_KEYS);
  });
  ...
  const toggleColVisible = useCallback((key) => {
    setColVisible((prev) => {
      ...
      try { localStorage.setItem('bom:col:ebom', JSON.stringify([...next])); } catch {}
      return next;
    });
  }, []);
  ...
  const visibleColDefs = useMemo(
    () => colDefs.filter((col) => EBOM_FIXED_KEYS.has(col.key) || colVisible.has(col.key)),
    [colDefs, colVisible]
  );
  ...
  function exportCSV() {
    const exportCols = visibleColDefs.filter((c) => c.key !== '_drag' && c.key !== '_link');
    ...
    <div className="flex flex-col h-full bg-white dark:bg-gray-900">
      ...
      <div className="px-3 py-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 shrink-0 flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center gap-1 ml-auto">
          ...
          >전체 접기</button>
          <ColumnManager colDefs={colDefs} visible={colVisible} onToggle={toggleColVisible} />
          <button
            onClick={exportCSV}
                ...
                >
                  {visibleColDefs.map((col) => {
                    ...
                    if (col.key === '_purchase') {
                      if (row.isAssyRow) {
                        return <td key="_purchase" style={{ width: col.w, minWidth: col.w }} className="border-r border-gray-200 dark:border-gray-700" />;
                      }
                    ...
                    if (col.key === 'vendor') {
                      if (row.isAssyRow) {
                        return <td key="vendor" style={{ width: col.w, minWidth: col.w }} className="border-r border-gray-200 dark:border-gray-700" />;
                      }
                      return (
                        <td key="vendor" style={{ width: col.w, minWidth: col.w }}
                          className="border-r border-gray-200 dark:border-gray-700 px-0 py-0">
                          <EditableCell rowId={row.id} field="vendor" value={value} onUpdate={updateDrawingPart} query={q} className="text-gray-700 dark:text-gray-300" />
                        </td>
                      );
                    }
                    ...
                    // 편집 가능 — staNo/processType/spec는 부품 행에서 DB 영속화
                    const onUpdate = !row.isAssyRow && DRAWING_PERSIST_EDITABLE.has(col.key)
                      ? updateDrawingPart
                      : updateRow;
